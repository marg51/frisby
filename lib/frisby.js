...
Frisby.prototype._request = function (/* method [uri, data, params] */) {
  ...
  //
  this.current.itInfo = "\033[032m"+method.toUpperCase()+"\033[0m" + ' ' + outgoing.uri.replace(/https?:\/\/[^\/]+\//,'/').replace(/user_email=.*&/,'').replace(/user_token=.*&/,'').replace('user_type=Admin','');
...
beforeEach(function() {
  this.addMatchers({
    toHaveSchema: function(schema) {
      var validation;
      this.message = function() {
        var msg = "# Schema validation failed #\n";
        for(err in validation.errors) {
          msg+="\t\033[36m*\033[0m \033[34m"+validation.errors[err].property+'\033[31m ' + validation.errors[err].message + '\033[0m\n';
        }

        return [msg,"Schema valid, expected not valid"]
      }
      // we know it will fail, so whats the point
      if(this.env.currentSpec.suite.statusCodeFailed) {
        return true;
      }

      validation = validate(this.actual,schema)
      return validation.errors.length == 0;
    },
    toHaveStatusCode: function(statusCode) {
      this.message = function() {
        return "expected status code to be "+statusCode+", but was "+this.actual;
      }
      this.env.currentSpec.suite.statusCodeFailed = this.actual != statusCode;
      return this.actual == statusCode;
    }
  })
})
...
Frisby.prototype.expectSchema = function(schema) {
  var self = this
  this.current.expects.push(function() {
    ...
    expect(jsonBody).toHaveSchema(schema)
  })
  return this;
}
Frisby.prototype.expectStatusCode = function(statusCode) {
  var self = this;
  this.current.expects.push(function() {
    expect(self.current.response.status).toHaveStatusCode(statusCode);
...
Frisby.prototype.toss = function(retry) {
  ...
  describe('ENDPOINT: ' + self.current.describe, function() {
